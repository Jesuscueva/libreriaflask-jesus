...
class AutorController(Resource):
    ...
    def put(self, id):
        autorEncontrado = AutorModel.query.filter_by(autorId=id).first()
        #no siemore es necesaria hacer la validación que el objeto exista puesto que el front se debe encargar de hacer esta validación
        if autorEncontrado:
            data = serialazer.parse_args()
